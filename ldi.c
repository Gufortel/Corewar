...
/*   Updated: 2019/01/09 20:35:14 by gufortel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ldi(t_env *p, t_proc *c)
{
	...
	|| get_01(p->mp[c->pc + 1].v) == 2) && (get_02(p->mp[c->pc + 1].v) == 1))
	{
		...
		adr1 = get_all(p, get_00(p->mp[c->pc + 1].v), c, 1);
		adr2 = get_all(p, get_01(p->mp[c->pc + 1].v), c, tt);
		adr1 = (adr1 + adr2) % MEM_SIZE;
		tt = (get_01(p->mp[c->pc + 1].v) == 1) ? tt + 1 : tt;
		tt = (get_01(p->mp[c->pc + 1].v) == 2) ? tt + 3 : tt;
		tt = (get_01(p->mp[c->pc + 1].v) == 3) ? tt + 2 : tt;
